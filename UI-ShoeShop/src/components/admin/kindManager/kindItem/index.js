    ...
    if (numberCategorySub === 0) {
      ...
      result[0] = (
        <TableRow key = {0}>
          ...
          <TableCell>
            <EditIcon className={classes.icon}> </EditIcon>{" "}
            <DeleteIcon className={classes.icon} onClick = {()=>remove(category.children[0]._id)}> </DeleteIcon>
          </TableCell>
        </TableRow>
      );
      ...
      for (var i = 1; i < numberCategorySub; i++) {
        result[i] = (
          <TableRow key = {i}>
            ...
            <TableCell>
              <EditIcon className={classes.icon}> </EditIcon>{" "}
              <DeleteIcon className={classes.icon} onClick = {()=>remove(category.children[i]._id)}> </DeleteIcon>
            </TableCell>
          </TableRow>
        );
      }
...
export default connect(null, dispatchMapToProps) (KindItem);
