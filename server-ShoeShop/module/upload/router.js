...
const {tempPath, serverPath, pathAvatar, contributePath} = require('../common/constant')
...
router.post('/images/single', upload(tempPath).single('image'), async function(req, res, next) {
  try {
    const file = req.file
 ...
 *       - bearerAuth: []
 */
router.post('/avatar', upload(pathAvatar).single('image'), async function(req, res, next) {
  try {
    const file = req.file
    if(!file) throw new Error('Image is missing ')
 ...
 *       - bearerAuth: []
 */
router.post('/images/multiple', upload(tempPath).array('images', 10), async function(req, res, next) {
  try {
 ...
 *       - bearerAuth: []
 */
router.post('/images/contribute', upload(contributePath).array('images', 10), async function(req, res, next) {
  try {
    const files = req.files
    if(files.length <= 0) throw new Error('Image is missing')
    res.json(makeResponse(files.map(e => e.filename))) 
